...
use crate::{
    AGENT_TESTING, EvaluationDecision, EvaluationStep, ExecutionState, ExecutionStatus, McpClient,
...
impl AgentKernel {
    ...
    pub async fn run(&mut self, goal: String) -> Result<Value> {
        ...
        while self.state.current_step < self.state.plan.len() {
            ...
            // Binding Phase
            let binding =
                StepBinding::resolve_binding(step, &self.executor, &self.state.context, &prompt)
                    .await?;
            ...
            match evaluation.decision {
                EvaluationDecision::Continue => {
                    ...
                    if step.final_output.is_some() {
                        self.final_output.insert(
                            step.final_output.clone().unwrap(),
                ...
                EvaluationDecision::Wait => {
                    self.state.status =
                        ExecutionStatus::Waiting(step_result.observation.unwrap().clone());
                }
                EvaluationDecision::Replan => {
                    self.state.status = ExecutionStatus::RePlanning(
                        step_result.observation.as_ref().unwrap().clone(),
                    );
                    self.state.plan = PlanStep::re_plan(
                        &self.planner,
                        &prompt,
                        &self.state.context,
                        step_result.observation.unwrap().clone(),
                    )
                    .await?;
                    continue;
                }
        ...
        match &step.action {
            StepActions::ToolCall { server, tool } => {
                ...
                match client.call_tool(tool, params).await {
                    Ok(response) => StepExecutionResult {
                        success: true,
                        output: response,
