...
WORKDIR /app/resource_service
COPY --from=build /app/resource_service/target/release/resource_service /usr/local/bin/resource_service
COPY --from=build /app/resource_service/target/release/resource_worker /usr/local/bin/resource_worker
