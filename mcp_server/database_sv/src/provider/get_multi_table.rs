...
impl SchemaProvider {
    pub async fn get_multi_table_schema(&mut self, table_names: Vec<String>)
    -> String
    {
    ...
    {
        let table_name = schema.get("table_name").unwrap().as_str().unwrap().to_string();
        ...
        for col in columns {
            ...
            let line = format!("==[   {} - {} - {} - {}   ]==", name, type_info, nullable, default_info);
