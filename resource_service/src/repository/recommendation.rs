...
impl ResourceRepository {
    pub async fn recommend(
        ...
        let rows = client
            .query(
                "SELECT r.id AS resource_id, r.canonical_url, r.title, r.difficulty::text,
                 ...
                 WHERE r.status = 'active'
                   ...
                   AND ($2::text IS NULL OR r.difficulty::text IN ($2::text, 'mixed', 'unknown'))
                 GROUP BY r.id
                 ORDER BY
                   r.is_official DESC,
            ...
            .await?;
        if !rows.is_empty() {
            return Ok(rows.iter().map(row_to_recommended_resource).collect());
        }
        ...
        Ok(fallback_rows
            .iter()
            .map(row_to_recommended_resource)
            .collect())
    }
}
